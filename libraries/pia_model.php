<?php
	...
	abstract class pia_model extends model {
		public function get_pia_case($pia_id) {
			$query = "select * from pia_cases where id=%d and user_id=%d limit 1";

			if (($result = $this->db->execute($query, $pia_id, $this->user->id)) == false) {
				return false;
			}
		...
		public function trace_pia($pia_id) {
			...
			$query = "select p.*, r.number from pia p, pia_rules r, pia_cases c ".
			         "where p.pia_rule_id=r.id and p.pia_case_id=c.id and c.id=%d and c.user_id=%d";
			if (($result = $this->db->execute($query, $pia_id, $this->user->id)) === false) {
				return false;
			}
