	...
	class casus_model extends process_model {
		public function get_cases() {
			$query = "select c.id, c.name, c.organisation, c.iso_standard_id, UNIX_TIMESTAMP(c.date) as date, s.name as iso_standard, ".
			         ...
			         "where c.iso_standard_id=s.id and c.user_id=%d and visible=%d order by date desc";

			return $this->db->execute($query, $this->user->id, YES);
		}
		...
		public function save_oke($case) {
			...
			if ($result) {
				$query = "select count(*) as count from cases where name=%s and organisation=%s and user_id=%d";
				$params = array($case["name"], $case["organisation"], $this->user->id);
		...
		public function create_case($case) {
			$keys = array("id", "user_id", "iso_standard_id", "name", "organisation", "date", "scope", "impact", "logo", "visible");

			$case["id"] = null;
			$case["user_id"] = $this->user->id;
			if (trim($case["logo"]) == "") {	
				$case["logo"] = null;
			}
			$case["visible"] = YES;
		...
		public function set_visibility($visibility) {
			$query = "update cases set visible=%d where user_id=%d";
			if ($this->db->query($query, NO, $this->user->id) === false) {
				return false;
			}
			...
			$query = "update cases set visible=%d where user_id=%d and id in (".implode(", ", $values).")";
			return $this->db->query($query, YES, $this->user->id, $visibility) !== false;
		}

		public function show_all_cases() {
			$query = "update cases set visible=%d where user_id=%d";
			return $this->db->query($query, YES, $this->user->id) !== false;
		}
