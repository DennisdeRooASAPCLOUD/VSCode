<?php
	class cms_user_model extends model {
		...
		public function get_users($order, $offset, $limit) {
			$query = "select u.*, (select count(*) from cases where user_id=u.id) as cases, ".
			                     "(select count(*) from pia_cases where user_id=u.id) as pias ".
			         "from users u ";
			$args = array();
		...
		public function delete_user($user_id) {
			...
			array_push($queries,
				array("delete from pia where pia_case_id in (select id from pia_cases where user_id=%d)", $user_id),
				array("delete from pia_cases where user_id=%d", $user_id),
				array("delete from case_bia_threat where case_id in (select id from cases where user_id=%d)", $user_id),
				array("delete from case_threat where case_id in (select id from cases where user_id=%d)", $user_id),
				array("delete from bia where case_id in (select id from cases where user_id=%d)", $user_id),
				array("delete from overruled where case_id in (select id from cases where user_id=%d)", $user_id),
				array("delete from cases where user_id=%d", $user_id),
