	...
	class pia_casus_model extends pia_model {
		public function get_pia_cases() {
			$query = "select id, name, UNIX_TIMESTAMP(date) as date ".
			         "from pia_cases where user_id=%d order by date desc";
		...
		public function save_oke($case) {
			...
			if (trim($case["name"]) == "") {
				...
				$query = "select count(*) as count from pia_cases where name=%s and user_id=%d";
				$params = array($case["name"], $this->user->id);
		...
		public function create_pia_case($case) {
			$keys = array("id", "user_id", "name", "date", "description");
